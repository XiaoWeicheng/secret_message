...
public class MessageGenerators {
    ...
    public Message generate(String msgId, MessageType type, Object data, ClientDelegate clientDelegate){
	    try {
		    byte[] encryptedData = messageGeneratorMap.get(type.getEncryptType()).generate(msgId, type, data,clientDelegate);
	...
	public <T> T parseMessage(Message message, Class<T> tClass, ClientDelegate clientDelegate) {
		try {
			byte[] originData = messageGeneratorMap.get(message.getType().getEncryptType()).parseMessage(message,clientDelegate);
